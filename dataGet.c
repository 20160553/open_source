...
#define DB_USER "open"
#define DB_PASS "dhvmsthtm123"
#define DB_NAME "open_source"
...
void infoShow(MYSQL *connection, char query[], char *name)
{
  ...
  else
  {
    ...
    if (!i)
      printf("info failed : %s is not exist.\n", name);
    mysql_free_result(res);
  }
}
...
void alarmShow(MYSQL *connection, char query[], char sname[], int threshold)
{
  ...
  else
  {
    ...
    while ((row = mysql_fetch_row(res)) != NULL)
    {
      if (strcmp(sname, row[0]) == 0 && threshold < atoi(row[2]))
      {
        printf("\r\n");
